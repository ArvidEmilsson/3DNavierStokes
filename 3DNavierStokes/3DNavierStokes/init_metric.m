function params = init_metric(params) 
    ...
    params.geom.m(:,:,:,1,1)      =  D_eta(Y.*z3)./W_eta  +Ly*z3+Lz*y2 +Ly*Lz -  D_zeta(Y.*z2)./W_zeta ;
    %params.geom.m12      = z2.*x3  -  x2.*z3  ;
    %params.geom.m12      = z2.*x3  -  x2.*z3  - x2*Lz;
    params.geom.m(:,:,:,1,2)      =  D_eta(Z.*x3)./W_eta   -  D_zeta(Z.*x2)./W_zeta - x2*Lz ;
    %params.geom.m13      = x2.*y3  -  y2.*x3   ;
    %params.geom.m13      = x2.*y3  -  y2.*x3  - Ly x3;
    params.geom.m(:,:,:,1,3)      =  D_zeta(x2.*Y)./W_zeta -  D_eta(x3.*Y)./W_eta   - Ly*x3 ;

    %params.geom.m21      = y3.*z1  -  z3.*y1 ;
    %params.geom.m21      = y3.*z1  -  z3.*y1 -Lz y1;
    params.geom.m(:,:,:,2,1)      = D_zeta(Y.*z1)./W_zeta  -  D_xi(z3.*Y)./W_xi     -Lz*y1  ;
    %params.geom.m22      = z3.*x1  -  x3.*z1 ;
    %params.geom.m22      = z3.*x1  + z3 *Lx  + Lz x1 + Lx Lz -  x3.*z1  ;
    params.geom.m(:,:,:,2,2)      = D_zeta(Z.*x1)./W_zeta  + z3*Lx  + Lz*x1 + Lx*Lz -  D_xi(x3.*Z)./W_xi     ;
    %params.geom.m23      = x3.*y1  -  y3.*x1 ;
    %params.geom.m23      = x3.*y1  -  y3.*x1 -y3 Lx ;    
    params.geom.m(:,:,:,2,3)      = D_xi(x3.*Y)./W_xi      -  D_zeta(Y.*x1)./W_zeta -y3*Lx ;
 
    %params.geom.m31      = y1.*z2  -  y2.*z1 ;
    params.geom.m(:,:,:,3,1)      = D_xi(Y.*z2)./W_xi      -  D_eta(Y.*z1)./W_eta  -Ly*z1  ;
    %params.geom.m32      = z1.*x2  -  z2.*x1 ;
    params.geom.m(:,:,:,3,2)      = D_xi(Z.*x2)./W_xi      -  D_eta(Z.*x1)./W_eta  -z2*Lx  ;
    %params.geom.m33      = x1.*y2  -  x2.*y1 ;
    params.geom.m(:,:,:,3,3)      = D_eta(x1.*Y)./W_eta  + Lx*y2 + x1*Ly + Lx*Ly -  D_xi(x2.*Y)./W_xi      ;

    %keyboard
    
    
    params.geom.Jacobian = params.geom.m(:,:,:,3,1).*x3 + ...
                          +params.geom.m(:,:,:,3,2).*y3 + ... 
                          +params.geom.m(:,:,:,3,3).*(z3 +Lz) ; 
                      
    params.geom.r_xi = zeros([ params.geom.n, 3,3,] ) ; 
    
    params.geom.r_xi(:,:,:,1,1) =  x1 ;  
    params.geom.r_xi(:,:,:,2,1) =  y1 ;  
    params.geom.r_xi(:,:,:,3,1) =  z1 ;  
    
    params.geom.r_xi(:,:,:,1,2) =  x2 ;  
    params.geom.r_xi(:,:,:,2,2) =  y2 ;  
    params.geom.r_xi(:,:,:,3,2) =  z2 ;  

    params.geom.r_xi(:,:,:,1,3) =  x3 ;  
    params.geom.r_xi(:,:,:,2,3) =  y3 ;  
    params.geom.r_xi(:,:,:,3,3) =  z3 ;  
